...
public sealed class TunnelPipelineIntegrationTests
{
    ...
    [Fact]
    public async Task WebSocketCoverProfileTransfersPaddedPacketsInBothDirections()
    {
        using var listener = new TcpListener(IPAddress.Loopback, 0);
        listener.Start();
        ...
        using (var clientPackets = new TestPacketEndpoint())
        using (var serverPackets = new TestPacketEndpoint())
        await using (TunnelPipeline clientPipeline = TunnelProfileFactory.CreateWebSocketCover("test"))
        await using (TunnelPipeline serverPipeline = TunnelProfileFactory.CreateWebSocketCover("test"))
        using (var stop = new CancellationTokenSource(TimeSpan.FromSeconds(5)))
        {
            Task clientRun = clientPipeline.RunAsync(clientPackets, client.GetStream(), stop.Token);
            Task serverRun = serverPipeline.RunAsync(serverPackets, server.GetStream(), stop.Token);
            byte[][] requests = Packets(0x20);
            byte[][] responses = Packets(0x80);
            ...
            Assert.Equal(SortedHex(requests), SortedHex(
                await ReadPacketsAsync(serverPackets, stop.Token)));
            Assert.Equal(SortedHex(responses), SortedHex(
                await ReadPacketsAsync(clientPackets, stop.Token)));
